...
import { IconButton, Divider, Card, CardContent, Button, Badge, Typography, Grid } from '@material-ui/core/';
...
class Details extends Component {
    ...
    addToCartHandler = (event, id, type, name, price) => {
        ...
        if (this.state.orderItems.items !== undefined && this.state.orderItems.items.some(item => (item.name === name))) {
            let index = this.state.orderItems.items.findIndex(element => element["name"]===name)
            let quantity = this.state.orderItems.items[index].quantity + 1;
            let priceForAll = this.state.orderItems.items[index].priceForAll + this.state.orderItems.items[index].pricePerItem;
    ...
    incrementItemQuantity = (item, index) => {
        let itemIndex = this.state.orderItems.items.findIndex(element => element["name"] === item.name)
        let quantity = this.state.orderItems.items[itemIndex].quantity + 1;
        let priceForAll = this.state.orderItems.items[itemIndex].priceForAll + this.state.orderItems.items[itemIndex].pricePerItem;
                ...
                <div className="main">
                    <div className="restaurant-details">
                        <div className="image-container">
                            <img src={this.state.photoUrl} alt="none" className="restaurant-image" />
                        </div>
                                        ...
                                        <Grid container key={item.id} style={{ marginBottom: 5 }}>
                                            ...
                                            <Grid item xs={5} lg={6}>
                                                <Typography>
                                                    <span
                                                        className="item-name">  
                                                        {/* To Pascal Case */}
                                                        {item.item_name.replace(/(\w)(\w*)/g, function (g0, g1, g2) { return g1.toUpperCase() + g2.toLowerCase(); })} </span>
                                                </Typography>
                                            </Grid>
                                            <Grid item xs={3} lg={2}>
                                                <div className='price'>
                                            ...
                                            <Grid item xs={2} lg={2}>
                                                <IconButton style={{ padding: 0, float: 'left' }}
                                                    onClick={(event) => this.addToCartHandler(event, item.id, item.item_type, item.item_name, item.price)}>
                                ...
                                <CardContent>
                                    <div style={{ fontWeight: "bold" }}>
                                        <span style={{ paddingRight: "20px" }}>
                                            <Badge className="cart-counter" badgeContent={this.state.totalItemCount}
                                                    ...
                                                    this.state.orderItems.items.map((item, index) => (
                                                        <Fragment key={item.id}>
                                                            ...
                                                            <Grid item xs={3} lg={4}>
                                                                <Typography>
                                                                    {/* Pascal case */}
                                                                    {item.name.replace(/(\w)(\w*)/g, function (g0, g1, g2) { return g1.toUpperCase() + g2.toLowerCase(); })}
                                                                </Typography>
                                                            </Grid>
                                                            <Grid item xs={3} lg={3} style={{ flexWrap: "wrap" }}>
                                                                <div className='add-remove-icon'>
                                                                    <IconButton className='add-remove-btn'
                                                                        style={{ display: "flex", padding: 0 }}
                                                                        onClick={(event) => this.decrementItemQuantity(event, item.id, item.type, item.name, item.pricePerItem)}>
                                                                            <RemoveIcon
                                                                                fontSize='default'
                                                                    ...
                                                                    <Typography
                                                                        style={{ fontWeight: 'bold' }}>{item.quantity}
                                                                    </Typography>
                                                                    <IconButton className='add-remove-btn'
                                                                        style={{ display: "flex", padding: 0 }}
                                                                        onClick={this.incrementItemQuantity.bind(this, item, index)}>
                                                                        <AddIcon 
                                                                            fontSize='default' 
                                                                            style={{
                                                                            color: 'black',
                                                            ...
                                                            <Grid item xs={4} lg={3}>
                                                                <span style={{ float: 'right' }}>
                                                                    <i className="fa fa-inr" aria-hidden="true"></i>
                                                                    <span
                                                                        style={{ paddingLeft: "2px" }}>{item.priceForAll.toFixed(2)}
                                                                    </span>
                                                                </span>
                                                            </Grid>
                                                        </Fragment>
                    ...
                    <StyledSnackBar open={this.state.itemAddedNotification} onClose={this.closeSnackBar}
                        message="Item added to cart!" />
                    <StyledSnackBar open={this.state.isCartEmpty} onClose={this.closeSnackBar}
                        message="Please add an item to your cart!" />
                    <StyledSnackBar open={this.state.itemQuantityDecreased} onClose={this.closeSnackBar}
                        message="Item quantity decreased by 1!" />
                    <StyledSnackBar open={this.state.itemRemovedFromCart} onClose={this.closeSnackBar}
                        message="Item removed from cart!" />
                    <StyledSnackBar open={this.state.itemQuantityIncreased} onClose={this.closeSnackBar}
                        message="Item quantity increased by 1!" />
                    <StyledSnackBar open={this.state.userNotLoggedIn} onClose={this.closeSnackBar}
